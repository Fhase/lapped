...
app.get("/admin", requireAdmin, async (req, res, next) => {
  try {
    if (req.query.view === "laps") return res.redirect("/admin?view=leaderboard");
    ...
    const [analytics, tickets, rankings] = await Promise.all([
      ...
      showLeaderboard ? connectionRankings(req.connectedTokens) : Promise.resolve(null)
    ]);
    ...
    const extras = `${leaderboard ? leaderboardPanel(leaderboard, req.connectedTokens) : ""}${rankings ? rankingPanel(rankings) : ""}${ticketPanel(tickets)}`;
