...
let searchInput=document.querySelector('.search input');const cleanSearch=searchInput?.cloneNode(true);if(cleanSearch){searchInput.replaceWith(cleanSearch);searchInput=cleanSearch}const tableBody=document.querySelector('tbody'),pages=document.querySelector('.pages');let searchTimer;
...
async function loadAthletes(page=1){const params=new URLSearchParams({page:String(page),q:searchInput.value});const response=await fetch('/admin/athletes?'+params);if(!response.ok)return;const data=await response.json();renderAthletes(data);const query=searchInput.value?('?q='+encodeURIComponent(searchInput.value)):'';history.replaceState(null,'','/admin'+query+(query?'&':'?')+'page='+data.page)}
